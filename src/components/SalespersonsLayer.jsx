      ...
      try {
        ...
        if (result.status.code === 0) {
          const mappedSalespersons = result.data.map((salesperson) => ({
            id: salesperson.id,
            name: `${salesperson.firstName} ${salesperson.lastName}`, // Combine firstName and lastName
            email: salesperson.email,
            phoneNo: salesperson.phoneNo,
            region: salesperson.region?.name || "N/A", // Extract region name
            route: salesperson.route?.name || "N/A", // Extract route name
            dateCreated: salesperson.dateCreated?.split("T")[0] || "N/A",
            status: salesperson.blocked ? "Inactive" : "Active", // Map blocked to status
          }));
          ...
          setTotalItems(result.totalElements); // Use totalElements from response
        } else {
              ...
              >
                <thead className="table-light text-start small-text" style={{ fontSize: "15px" }}>
                  <tr>
                    ...
                    <th className="text-start py-3 px-4">Phone No.</th>
                    <th className="text-start py-3 px-4">Region</th>
                    <th className="text-start py-3 px-4">Route</th>
