...
const SubRegionDetails = () => {
  ...
  const subRegion = location.state?.subRegion || defaultSubRegion;
  ...
  const stats = [
    { title: "Total Employees", count: subRegion.salesAgents || 1007, icon: "mdi:account-group", color: "bg-dark" },
    { title: "Active", count: subRegion.salesAgents || 1007, icon: "mdi:account-check", color: "bg-success" },
  ...
  if (!subRegion) {
    return <div>No sub-region data found</div>;
  }
  ...
  const tables = {
    ...
    salesAgents: [
      { id: 1, name: "Agent X", subRegion: subRegion.name || "Nyanza" },
      { id: 2, name: "Agent Y", subRegion: subRegion.name || "Coastal" },
      { id: 3, name: "Agent Z", subRegion: subRegion.name || "Nairobi" },
    ],
  };
          ...
          <table className="table table-borderless text-start small-text">
            <thead className="table-light text-start small-text">
              <tr>
                <th className="text-start">#</th>
                <th className="text-start">Name</th>
                <th className="text-start">Sub Region</th>
              </tr>
            </thead>
            <tbody>
              {tables.salesAgents.map((agent) => (
                <tr key={agent.id}>
                  ...
                  <td className="text-start small-text">{agent.subRegion}</td>
                </tr>
              ))}
            </tbody>
          </table>
