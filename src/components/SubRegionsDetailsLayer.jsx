...
const SubRegionDetails = () => {
  ...
  const { subRegion } = location.state || { subRegion: { name: "Default Sub Region" } }; // Fallback for SubRegion
  ...
  if (!subRegion) {
    return <div>No Sub Region data found for {subregionName}</div>;
  }
  ...
  const tables = {
    ...
    salesAgents: [
      { id: 1, name: "Agent X", subRegion: "Nyanza" },
      { id: 2, name: "Agent Y", subRegion: "Coastal" },
      { id: 3, name: "Agent Z", subRegion: "Nairobi" },
    ],
  };
    ...
    <div className="page-wrapper">
      ...
      <div className="card shadow-sm mt-3 full-width-card">
        <div className="card-body">
          ...
          {renderTable()}
          {/* <Link to="/sub-regions" className="btn btn-primary">
            Back
          </Link> */}
        </div>
      </div>
