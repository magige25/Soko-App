...
const AddDepotLayer = ({ onDepotAdded }) => {
    ...
    const fetchRegionsAndSubRegions = async () => {
      try {
        ...
        const [regionsResponse, subRegionsResponse] = await Promise.all([
          axios.get(REGIONS_API_URL, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(SUBREGIONS_API_URL, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        ...
        console.error('Error fetching regions/subregions:', error);
        setErrors({ fetch: 'Failed to load regions and subregions' });
      }
    };
  ...
  // Filter subregions when region changes
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(DEPOT_API_URL, payload, {
        headers: {
      ...
      if (response.data.status?.code === 0 && onDepotAdded && response.data.data) {
        const newDepot = {
        ...
        onDepotAdded(newDepot);
      }

      navigate('/depot');

    } catch (error) {
      console.error('Error adding depot:', error.response?.data || error.message);
      setErrors({
        submit: error.response?.data?.message || 
                error.message || 
                'Failed to add depot. Please check your connection and try again.',
      });
    } finally {
      setIsLoading(false);
    }
