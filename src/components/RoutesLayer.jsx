...
const RoutesLayer = () => {
  ...
  useEffect(() => {
    ...
    const resetEditForm = () => !isLoading && setEditRoute({ 
      id: null, 
      name: '', 
      subRegionId: '', 
      regionId: '', 
  ...
  const handleAddRoute = async (e) => {
    ...
    try {
      ...
      await fetchRoutes();
      setNewRoute({ name: '', subRegionId: '', regionId: '' }); // Reset form
      document.getElementById("addRouteModal").classList.remove("show"); // Close modal
          ...
          <div className="card-body">
              ...
              <table className="table table-borderless table-hover text-start small-text" style={{ width: "100%", fontSize: "15px" }}>
                <thead className="table-light text-start small-text">
                  <tr>
                    ...
                    <th className="text-start py-3 px-4">Sub-Region</th>
            ...
            <div className="d-flex justify-content-between align-items-center mt-3">
              <div className="text-muted">
                <span>Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredRoutes.length)} of {filteredRoutes.length} entries</span>
              </div>
                          ...
                          {subRegions.map((subRegion) => (
                            <li key={subRegion.id}>
                              <button
                                ...
                                className="dropdown-item"
                                onClick={() => {
                                  setEditRoute({ ...editRoute, subRegionId: subRegion.id, subRegion: subRegion.name });
                                  setShowSubRegionDropdown(false);
                                }}
                              >
                                {subRegion.name}
